...
const VoiceActivityComponent =
    (props) => {
      ...
      useEffect(() => {navigator.mediaDevices.getUserMedia({audio : true})
                           .then((stream) => {
          ...
          .catch((err) => {
            console.error(`Failed to add audio worklet: ${err}`)
                           ...
                           .catch((err) => {console.error(
                                      `Failed to get user media: ${err}`)})},
                [])

      useEffect(() => {
        if (!props.chunk || props.chunk.length === 0)
          return
            ...
            if (!recordingRef.current) {
              ...
              const options = {
                ...
                numberOfAudioChannels : 1,
                              ...
                              recorder.current =
                  RecordRTC(microphone.current, options)
